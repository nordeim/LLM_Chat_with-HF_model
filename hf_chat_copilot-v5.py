...
class ModelWrapper:
    """Wrapper class to provide a consistent interface for both pipeline and direct model approaches"""
    def __init__(self, model_id: str):
        ...
        if "EXAONE" in model_id:
            ...
            self.pipeline = pipeline("text-generation", model=model_id, max_new_tokens=150, trust_remote_code=True)
...
def get_model_pipeline(model_id: str):
    """Returns a ModelWrapper instance that provides a consistent generation interface"""
    try:
        return ModelWrapper(model_id)
...
def generate_response(user_message: str, image, chat_history, model_id: str):
    ...
    # Retrieve or load the model wrapper
    model_wrapper = get_model_pipeline(model_id)
    ...
    try:
        # Generate response using the wrapper's consistent interface
        output = model_wrapper.generate(conversation)
