...
def generate_response(user_message: str, image, chat_history, model_id: str):
    ...
    if gen_pipeline is None:
        error_message = f"Failed to load model: {model_id}. Please check if the model ID is correct and try again."
...
with gr.Blocks(css=css_str) as demo:
    ...
    gr.Markdown(
        ...
        "You can specify any Hugging Face model; the default is `ds4sd/SmolDocling-256M-preview`."
    )
